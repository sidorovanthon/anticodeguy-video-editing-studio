...
## 2026-04-29 — Correction to 2026-04-28 entry (HF gate flag landscape)
- The 2026-04-28 entry above states `inspect --strict-all`. In HF v0.4.31 the actual implemented form is `inspect --strict`; `lint`/`validate` silently accept `--strict-all` but do not honor it (forward-compat no-ops). The shipped wrapper `tools/scripts/run-stage2-compose.sh` carries `lint --strict-all`, `validate --strict-all`, and `inspect --strict`; `AGENTS.md` hard rule and the spec implementation note (commit 0f97c6c) reflect this. Discovered during implementation of Task 5; the 2026-04-28 entry was written before the discrepancy surfaced. Append-only rule preserved — no edit to the prior entry.
