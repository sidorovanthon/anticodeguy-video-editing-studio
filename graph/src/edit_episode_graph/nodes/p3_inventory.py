...
def p3_inventory_node(state, *, runner=_run):
    ...
    # ElevenLabs Scribe + take-pack fan-out. No-op when
    # ``HOMESTUDIO_STEP_DEBUG`` is unset. Note: Scribe is a PAID call —
    # the pre-interrupt fires BEFORE dispatch so the operator can abort
    # if needed (Phase A decision: pre-execution skip is not in scope;
    # an operator who's exhausted credits can abort).
    ...
    if _sd_enabled():
        return wrap_deterministic_node(
            ...
            inner=lambda: _p3_inventory_body(state, runner=runner),
        )
    return _p3_inventory_body(state, runner=runner)


def _p3_inventory_body(state, *, runner=_run):
    slug = state.get("slug")
    if not slug:
