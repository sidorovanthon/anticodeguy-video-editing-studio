...
**Six structural quirks discovered during HOM-347 / HOM-351 / HOM-358 / HOM-360 / HOM-359 deploy** that any future change to `deploy.sh` / Dockerfile generation / compose volumes must preserve:
...
5. The `video-use` and `hyperframes` skill trees are NOT in the image (skills are external, auto-updated on the dev box via Task Scheduler — baking them into the image would freeze them). Four Phase 3/4 graph constants hardcode the canonical paths under `Path.home() / ".claude" / "skills" / "video-use" / ...` (`p3_inventory.HELPERS_DIR`, `p3_render_segments.RENDER_PY`, `p3_self_eval.TIMELINE_VIEW_PATH`, `gates/animation_map._GLOBAL_FALLBACK`) with no env override. `deploy.sh::deploy_rebuild` rsyncs `~/.claude/skills/video-use/` and `~/.agents/skills/hyperframes/` to `/var/lib/homestudio-langgraph/skills/{video-use,hyperframes}/` on TrueNAS (`rsync -L` to dereference the dev-box symlink/junction that points at `~/repos/video-use`), and compose bind-mounts them read-only at `/root/.claude/skills/video-use` and `/root/.agents/skills/hyperframes`. Removing either the rsync step or the compose volume re-breaks `p3_inventory` with `missing video-use helper(s): /root/.claude/skills/video-use/helpers/transcribe_batch.py, ...`. Symmetric to the HOM-349 episodes/inbox bind-mount pattern. An env-var-override-for-the-four-constants follow-up is out of scope of HOM-360 (see ticket "Latent code-quality follow-up"). Discovered HOM-334 Group B audit, 2026-05-24 (HOM-360).
6. The `claude` CLI inside the container authenticates via **subscription OAuth, not API key** (`backends/claude.py:3` docstring: "Subscription auth only."). OAuth tokens land at `/root/.claude/` — same parent directory that hosts the HOM-360 read-only skill-tree child mount at `/root/.claude/skills/video-use`. `deploy.sh::ensure_dockerfile` tacks `npm i -g @anthropic-ai/claude-code` onto the HOM-358 nodejs RUN chain (must be in the SAME `&&` chain so `npm` is on PATH — a separate sed-after-FROM injection lands before nodejs and fails); `deploy.sh::deploy_rebuild` creates `/var/lib/homestudio-langgraph/claude-session/` (chmod 777, idempotent); compose bind-mounts that host dir to `/root/.claude:rw` (parent mount; the skill-tree child mount coexists because Docker handles parent+child binds independently — child shadows the parent path). One-off operator action after the first deploy: `ssh truenas 'sudo docker exec -it homestudio-langgraph-server claude login'` (browser OAuth flow, agent does NOT run this). Removing either piece — npm install in the Dockerfile, mkdir+chmod of the host dir, or the parent `:rw` volume — re-breaks subscription auth: every `docker compose up -d` wipes the in-image `/root/.claude/` and the first LLM dispatch fails with AuthError. Discovered HOM-334 Group E audit, 2026-05-24 (HOM-359).
