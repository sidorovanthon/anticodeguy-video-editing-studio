...
4. **Intra-run retry on transient backend errors (HOM-158).** Each LLM node carries a `langgraph.types.RetryPolicy(max_attempts=2, retry_on=(BackendTimeout,))` configured in `graph.py` (single shared `_LLM_RETRY_POLICY` constant). `RetryPolicy` only fires when a node *raises* — pre-HOM-158 `_llm.py` swallowed `AllBackendsExhausted` into a `state["errors"]` delta, which from pregel's view is a successful return, and the policy never engaged (`pregel/_retry.py:260` matches `except Exception` to dispatch). HOM-158 changes the contract: `_llm.py` re-raises on terminal failure, emitting a `llm_attempts_exhausted` custom event with per-attempt telemetry first so Studio still surfaces what was tried (state writes are NOT committed on raise — `state["llm_runs"]` is intentionally untouched on this path). The policy + cfg-fingerprint (mechanism 3) are the correct pair: `RetryPolicy` rides out a single Windows shim hiccup intra-run; if both attempts fail, pregel caches the `__error__` sentinel — and the next thread re-runs only when the operator bumps timeout/model/backend in `graph/config.yaml` (mechanism 3 invalidates the key). Doc: <https://langchain-ai.github.io/langgraph/reference/types/#langgraph.types.RetryPolicy>. We retry only on `BackendTimeout` — `AuthError`, `BackendCLIError`, and `SchemaValidationError` are deterministic and would just burn the same dollar twice.

**Routing implication (HOM-158).** Pre-HOM-158 every conditional-edge router started with `if state.get("errors"): return END`. Combined with `state["errors"]`'s `add` (append-only) reducer, that meant any historical error on the thread permanently routed every downstream conditional to END — observed as a poisoned-thread halt at TURN 2 of HOM-154 even after the operator fixed the underlying issue. After HOM-158:
- LLM-followup routers *drop* the check entirely. Pregel does not commit writes on raise, so LLM failures never appear in `state["errors"]`.
- Deterministic-followup routers (and pickup, glue_remap, scaffold, render_segments, catalog_scan, assemble_index, studio_launch) call `_predecessor_just_failed(state, "<predecessor>")`, which returns True only when the *latest* error came from the named predecessor. This preserves the "predecessor-just-failed → END" intent while ignoring stale errors from elsewhere in the graph.

Edge case: a deterministic predecessor that failed on a prior turn AND succeeds on the current turn without overwriting `state["errors"][-1]` still routes to END under the scoped check. Acceptable for v1 — deterministic failures are usually structural (missing files, bad input) and don't recover by retry; clearing state is the documented escape hatch. Option C from the HOM-158 ticket (split current/historical error channels via a non-`add` reducer) is the long-term fix and is tracked separately.
