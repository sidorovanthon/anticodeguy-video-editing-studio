...
| F3 | **Version drift** (memory 0.4.x · skill ~0.5.x · runtime 0.6.63). Frozen workarounds may be obsolete; "author beats inline" prohibition may be liftable. | **[NEW]** — not ticketed | §4.3; `npm view hyperframes version` = 0.6.63; loader rewrite in source |
...
| F8 | video-use fork divergence: our `fix(render)` landed upstream under a different sha → `git pull` won't fast-forward. Auto-updaters pull from **npm**, not the repo. | **[NEW]** — operational, low risk | `git log @{u}..HEAD`; scheduled-task actions |
