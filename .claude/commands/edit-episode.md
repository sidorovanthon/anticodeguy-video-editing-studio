...
>
> **Sub-composition split — unblocked on HF 0.6.63.** The canon-blessed `<template>` + `data-composition-src` pattern (per `SKILL.md` §"Composition Structure") works on HF 0.6.63: sub-comps render real content (no black frames) and `npx hyperframes compositions` reports correct element counts. The 0.4.x loader bug (#589) that previously forced inline authoring was bare-repro'd FIXED on 2026-05-31 (HOM-379: `docs/bare-repro-verdicts/2026-05-31-hom-379-hf-0663-revalidation.md`). You MAY split beats into `compositions/beat-{N}-{slug}.html` mounted via `data-composition-src`. (In the LangGraph pipeline — the primary entry point — Phase 4 still inlines per-scene fragments under the orchestrator-house Pattern A; the Pattern-A→B migration is tracked in HOM-383.)
>
