...
>
> **Sub-composition split — BLOCKED pending upstream fix.** Do NOT split beats into `compositions/beat-{N}-{slug}.html` mounted via `data-composition-src`. The canon-blessed `<template>` + `data-composition-src` pattern (per `SKILL.md:149-185`) currently produces black renders and `0 elements / 0.0s` from `npx hyperframes compositions` in HF 0.4.41 — the CLI loader does not unwrap `<template>.content`. Verified by Phase 0 bare-repro (PR #24 findings doc). For ≥ 3 beats: author the root `index.html` with all beats inline. Re-enable split once upstream issue https://github.com/heygen-com/hyperframes/issues/589 lands and we bump HF version. Tracking memory: `feedback_hf_subcomp_loader_data_composition_src.md` (will be written in Step 9).
>
